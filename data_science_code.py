...
# 7) Model choices — baseline logistic, RF, XGBoost
models = {
...
# 8) Hyperparameter tuning — RandomizedSearch with StratifiedKFold
param_dist = {
...
# 1️⃣ Extract the trained classifier from the pipeline
xgb_model = best_model.named_steps['clf']

# 2️⃣ Preprocess the training data using the fitted preprocessor
X_train_pre = best_model.named_steps['pre'].transform(X_train)

# 3️⃣ Get final feature names (after encoding)
...
# 4️⃣ Define a prediction function (required by KernelExplainer)
f = lambda x: xgb_model.predict_proba(x)[:, 1]

# 5️⃣ Initialize the SHAP KernelExplainer with a sample of the training data
# (using a subset keeps runtime manageable)
explainer = shap.KernelExplainer(f, X_train_pre_df.sample(100, random_state=42))

# 6️⃣ Compute SHAP values for a few samples (to avoid long computation time)
X_sample = X_train_pre_df.sample(10, random_state=42)
shap_values = explainer.shap_values(X_sample)

# 7️⃣ Global feature importance summary plot
...
# 1️⃣ Recreate the constraint object fresh
constraint = DemographicParity()

# 2️⃣ Initialize the mitigator again
mitigator = ExponentiatedGradient(
    estimator=LogisticRegression(max_iter=1000, solver='liblinear'),
    constraints=constraint
)

# 3️⃣ Train with fairness constraint
mitigator.fit(X_train_pre, y_train, sensitive_features=A_train)

# 4️⃣ Predict
y_pred_fair = mitigator.predict(X_test_pre)

# 7️⃣ Evaluate fairness metrics
...
#     pred = "❤️ High Risk of Heart Disease" if prob > 0.5 else "💚 Low Risk of Heart Disease"
# 
